...
HRESULT eventShipRight::init(float x, float y)
{
	// 적 배 이미지
	_enemy.image = IMAGEMANAGER->addImage("shipRight", "Scene/event/shipRight.bmp", 300 * 2.5, 300 * 2.5, true, RGB(255, 0, 255));
...
void eventShipRight::release(void)
{

}

void eventShipRight::update(void)
{
...
void eventShipRight::render(void)
{
	IMAGEMANAGER->findImage("shipRight")->render(getMemDC(), _enemy.x - 418, _enemy.y - 350);
	...
	IMAGEMANAGER->findImage("light64")->frameRender(getMemDC(), _enemy.x - 390, _enemy.y - 380, lightFrame64, 0);
	IMAGEMANAGER->findImage("lightFind32")->frameRender(getMemDC(), _enemy.x-80, _enemy.y - 75, lightFrameNum, 0);

	_enemy._bullet->render();

	// 총구 애니메이션
	if (gunState == gunIDLE2)
	{
		IMAGEMANAGER->findImage("canonWait")->frameRender(getMemDC(), _enemy.x - 310,
			_enemy.y - 100, gunCurrentX, gunCurrentY);
	}
	if (gunState == gunWAIT2)
	{
		IMAGEMANAGER->findImage("canonWait")->frameRender(getMemDC(), _enemy.x - 310,
			_enemy.y - 100, gunCurrentX, gunCurrentY);
	}
	if (gunState == gunSHOOT2)
	{
		IMAGEMANAGER->findImage("canonShoot")->frameRender(getMemDC(), _enemy.x - 310,
			_enemy.y - 100, gunCurrentX, gunCurrentY);
	}
...
void eventShipRight::shipShoot()
{
	...
	// 직접 쏠 총구 위치 지정. 
	_cannon.shootCenter.x = _enemy.x - 220;
...
void eventShipRight::gunFrameFunc(void)
{
	switch (gunState)
	{
	case gunIDLE2:
